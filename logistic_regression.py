...
def do_experiments(start, end, step_num):
    ...
    for filename in os.listdir(result_dir):
        if filename.startswith("dataset_") or filename.startswith("parameters_"):
            print("hiya!!")
            os.remove(os.path.join(result_dir, filename))
    dataset_filename = f"dataset_{uuid.uuid4().hex}.png"
    parameters_filename = f"parameters_vs_shift_distance_{uuid.uuid4().hex}.png"
    dataset_path = os.path.join(result_dir, dataset_filename)
    parameters_path = os.path.join(result_dir, parameters_filename)


    n_samples = step_num  # Use step_num for correct number of subplots
    n_cols = 2
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        logistic_loss = -np.mean(y * np.log(y_prob) + (1 - y) * np.log(1 - y_prob))
        ...
        for level, alpha in zip(contour_levels, alphas):
            ...
            if level == 0.7:
                class_1_contour = plt.contour(xx, yy, Z, levels=[level], colors=['red'], linewidths=0.5)
                class_0_contour = plt.contour(xx, yy, Z, levels=[1 - level], colors=['blue'], linewidths=0.5)
    ...
    # Plot other results across shift distances
    ...
    plt.savefig(parameters_path)
    plt.close()
    return dataset_filename, parameters_filename
